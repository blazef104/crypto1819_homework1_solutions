...
\end{itemize}
\todo{Complete oracle to ufcmva interaction}
\subsubsection{ ii }
To show that:
\[
...
\subsubsection{ iii }
Let's contruct our $\Pi'$ as:
\begin{itemize}
   \item Tag'(x)=Tag(x)||0
   \item Vrf'(k,m,$\varphi$) executes:
   \begin{enumerate}
      \item Splits $\varphi$ into 2 parts: $\varphi', i$.
      \item Uses the first part as input for Vrf(k,m,$\varphi'$) and stores the output into d.
      \item Now if d=1 check if $i \geq 1$ and if so outputs $k[i]$, the i-th bit of the key.
   \end{enumerate}
\end{itemize}
...
\begin{figure}[h!]
   ...
   \end{tikzpicture}
\end{figure}
To leak the key completely $A$ sends 
