...
Upon receiving $x^\prime$ $D$ can compute $G(x^\prime)=z^\prime$, now if $z^\prime=z$ $D$ will decide $b^\prime=1$, otherwise $b^\prime=0$. The probability 
of $D$ making the wrong guessing is related to the probability that $D^\prime$ does not correctly inverts the function. This happens with negligible probability
\todo{Conclude on the probability (which should be correct guess-wrong guess)}
 
(b) \\
(i) By contraddiction assume that $\exists$ a distinguisher $D^\prime_g$ that is able to invert $g$ with non negligible probability (as before).
We will then have:
...
Thereofre $D^\prime_f$ leaks f (on input $z$) with the same probability of $D^\prime_g$ which is non negligible. $\implies$ Contraddiction.

\vspace*{\baselineskip}
\noindent(ii) Let $g$ be a OWF then we can define $f$ as follows:
...
distribution while the other one not.\\
Intuitively we can state that, since the XOR operation preserves the
\textit{highest randomness} among the two operands, $z=G_{1}(.) \xor G_{2}(.)$ has
uniform distribution.\\

Hence, let's define
\[
    G^{*}(x, x')=G_{1}(x) \xor G_{2}(x')
\]
for $(x, x') $ chosen randomly over the key space.\\

Now , we want to show that, given a PRG function and a non-PRG function,
$G(x)=G_{1}(x)\xor G_{2}(x)$ is a PRG.\\
Suppose that we have the following game, where the PRG function is $G_{1}$:
...
\begin{figure}[h!]
   ...
   \begin{tikzpicture}
      ...
      \node[anchor=west] at (mess from) {\shortstack[l]{
      		$ z=G_{1}(x),$ for $ x \leftarrow\$ U_{\lambda}  $ 
            \\
            $  z \leftarrow\$ U_{\lambda + l}  $
    }};

      \postlevel
      \mess{B}{$G^{*}=z\xor G_{2}(x')=z$}{A}
      \node[anchor=west] at (mess to) {  };
...
, with $x' \leftarrow\$ \{0,1\}^{\lambda}$ and $G_{2}$ such that its output is
always equal to $0^{n}$.
Then $G^{*}=z \xor G_{2}(x')=z$ always.
Now $D$ wins over $C^{prg}$ , but this is a contraddiction since PRG cannot be
distinguished from $U_{\lambda + l}$.\\
Furthermore, since the same reduction can be built for the case when $G_{2}$ is
the PRG one, we can state that $G^{*}$ is always a PRG.\\
\newpage
\subsection{point b}
To improve seed length we have to show that, for all possible $G_{1}$ and
$G_{2}$ with one of them PRG and the other not and if we choose always the same
input for both of them, $G^{*}=G_{1}(x) \xor G_{2}(x)$ is a PRG.\\
\todo{TO REVIEW}
======================================================================\\
There exists a critical example:$G^{*}(x)=G_{1}(x) \xor G_{2}(x)$ and
$G_{1}=G_{2} \xor 1^{n} $, hence $G^{*}(x)= G_{2}(x) \xor 1 \xor G_{2}(x) = 1
\Rightarrow$ not PRG.\\

ATTENZIONE: fare lo XOR preserva la distribuzione "migliore", ossia quella più
vicina all'uniformità. Se faccio lo XOR tra 1 e la PRG, la funzione $G_{1}$ è
una PRG a sua volta (quando invece dovrebbe essere una NON-prg).\\
Cioè, $G_{1}$ NON-PRG è tale anche senza guardare al risultato di $G_{2}$, mentre
così sembra che per ottenere un valore "predicibile" di $G_{1}$ devo sempre
guardare prima a $G_{2}$.
\\======================================================================\\
